...
public final class SortOverrideButton extends Button implements ITooltip {
    ...
    private Blitter background() {
        if (isHovered()) {
            return Blitter.icon(Icon.TOOLBAR_BUTTON_BACKGROUND_HOVER);
        }
        if (isFocused()) {
            return Blitter.icon(Icon.TOOLBAR_BUTTON_BACKGROUND_FOCUS);
        }
        return Blitter.icon(Icon.TOOLBAR_BUTTON_BACKGROUND);
    }

    private Blitter icon() {
        return switch (this.value) {
            case DEFAULT -> Blitter.icon(Icon.POWER_UNIT_AE);
